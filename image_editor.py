...
class ImageEditorWindow(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        self.title("محرر الصور المتقدم - AI Vision Pro")
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.sidebar, text="AI Vision Pro", font=ctk.CTkFont(size=22, weight="bold")).grid(row=0, column=0, padx=20, pady=(20, 10))

        btn_font = ctk.CTkFont(size=13)
        ctk.CTkButton(self.sidebar, text=fix_arabic("📁 فتح صورة"), font=btn_font, command=self.load_image).grid(row=1, column=0, padx=20, pady=5, sticky="ew")
        ctk.CTkButton(self.sidebar, text=fix_arabic("💾 حفظ النتيجة"), font=btn_font, fg_color="#10b981", command=self.save_image).grid(row=2, column=0, padx=20, pady=5, sticky="ew")
        
        ctk.CTkLabel(self.sidebar, text=fix_arabic("التحكم الثابت"), font=ctk.CTkFont(size=13, weight="bold"), text_color="gray").grid(row=3, column=0, pady=(25, 5))
        ctk.CTkButton(self.sidebar, text=fix_arabic("↩️ تراجع"), fg_color="#475569", command=self.undo).grid(row=4, column=0, padx=20, pady=5, sticky="ew")
        ctk.CTkButton(self.sidebar, text=fix_arabic("↪️ إعادة"), fg_color="#15803d", command=self.redo).grid(row=5, column=0, padx=20, pady=5, sticky="ew")
        ctk.CTkButton(self.sidebar, text=fix_arabic("🔄 إعادة للأصل"), fg_color="#c2410c", command=self.reset_image).grid(row=6, column=0, padx=20, pady=5, sticky="ew")
        
        self.btn_compare = ctk.CTkButton(self.sidebar, text=fix_arabic("👁️ اضغط للمقارنة"), fg_color="#4b5563")
        self.btn_compare.grid(row=7, column=0, padx=20, pady=15, sticky="ew")
        self.btn_compare.bind("<ButtonPress-1>", self.show_original_temp)
        self.btn_compare.bind("<ButtonRelease-1>", self.show_current_temp)
        ...
        self.image_display = ctk.CTkLabel(self.main_frame, text=fix_arabic("الرجاء تحميل صورة"), text_color="gray", font=ctk.CTkFont(size=18))
        ...
        ai_models = ["بدون ذكاء اصطناعي", "اكتشاف الوجوه (Face Detection)", "تتبع وتشويش الوجوه (Privacy)", "التعرف على الأشياء (MobileNet)"]
        for m in ai_models: ctk.CTkRadioButton(self.tools_panel.tab(fix_arabic("الذكاء الاصطناعي")), text=fix_arabic(m), variable=self.ai_var, value=m, command=self.update_preview).pack(pady=10, padx=10, anchor="w")
        ...
        ctk.CTkButton(self.tools_panel.tab(fix_arabic("الألوان")), text=fix_arabic("✅ تثبيت الألوان (للصور)"), command=self.apply_sliders_to_working).pack(pady=5, fill="x", padx=20)
        ...
        effects = [
            "بدون تأثير", "تدرج رمادي", "عكس الألوان (Invert)", "تمويه (Gaussian)", "تمويه (Average)", "إزالة الضوضاء (Median)", 
            "زيادة الحدة (Laplacian Sharpening)", "كشف الحواف (Canny)", "حواف (Sobel X+Y)", "حواف (Prewitt)", "حواف (Laplacian)", 
            "بوصلة كيرش (Kirsch 8-Directions)", "بوصلة روبنسون (Robinson)"
        ]
        ...
        # --- 4. المورفولوجي ---
        morphs = [("تآكل (Erosion)", "erosion"), ("تمدد (Dilation)", "dilation"), ("فتح (Opening)", "opening"), 
                  ("إغلاق (Closing)", "closing"), ("تدرج مورفولوجي (Gradient)", "gradient"), ("قبعة علوية (Top Hat)", "tophat"), ("قبعة سوداء (Black Hat)", "blackhat")]
        ...
        seg_options = [
            "بدون تقطيع", "اكتشاف الخطوط (Hough Lines)", "اكتشاف الدوائر (Hough Circles)", "اكتشاف الزوايا (Shi-Tomasi)", 
            "تقطيع (Otsu Threshold)", "تقطيع متكيف (Adaptive Mean)", "تقطيع متكيف (Adaptive Gaussian)",
            "عزل اللون الأحمر (Color Masking)", "تقطيع (K-Means K=3) - للصور", "خوارزمية (Watershed) - للصور"
        ]
    ...
    def apply_processing(self, frame):
        ...
        if seg_mode != "بدون تقطيع":
            ...
            elif seg_mode == "تقطيع (K-Means K=3) - للصور":
                if len(img.shape) == 3:
                    Z = img.reshape((-1, 3))
                    Z = np.float32(Z)
            ...
            elif seg_mode == "خوارزمية (Watershed) - للصور":
                if len(img.shape) == 3:
    ...
    def show_original_temp(self, event):
        if self.original_image is not None: self.render_image(self.original_image)
    def show_current_temp(self, event):
        if self.current_image is not None: self.render_image(self.current_image)
    def on_resize(self, event):
        if self.current_image is not None: self.after(100, lambda: self.render_image(self.current_image))
